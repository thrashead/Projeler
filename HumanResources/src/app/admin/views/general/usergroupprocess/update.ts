...
export class AdminUserGroupProcessUpdateComponent {
	...
	onSubmit() {
		...
		this.service.post("UserGroupProcess", "Update", this.data).subscribe((answer: any) => {
			...
			this.deleteShow = AdminLib.UserRight(userRights, Model, "d");
				...
				if (this.callTable == true) {
					this.route.params.subscribe((params: Params) => {
						this.id = params['id'];
						this.subscription = this.service.get("UserGroupProcess", "Update", this.id).subscribe((resData: any) => {
							this.model = resData;
