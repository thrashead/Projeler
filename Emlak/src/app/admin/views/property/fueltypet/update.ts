...
export class AdminFuelTypeTUpdateComponent {
	errorMsg: string;
	id: string;
	...
	FillData() {
		if (this.callTable == true) {
			this.route.params.subscribe((params: Params) => {
				this.id = params['id'];
				this.subscription = this.service.get("FuelTypeT", "Update", this.id).subscribe((answer: any) => {
					this.model = answer;
	...
	onSubmit() {
		...
		this.service.post("FuelTypeT", "Update", this.data)
