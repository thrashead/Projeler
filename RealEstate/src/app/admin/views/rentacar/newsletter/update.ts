...
export class AdminNewsletterUpdateComponent {
	errorMsg: string;
	id: string;
	...
	FillData() {
		if (this.callTable == true) {
			this.route.params.subscribe((params: Params) => {
				this.id = params['id'];
				this.subscription = this.service.get("Newsletter", "Update", this.id).subscribe((answer: any) => {
					this.model = answer;
	...
	onSubmit() {
		...
		this.service.post("Newsletter", "Update", this.data)
