...
export class AdminWorkersTUpdateComponent {
	errorMsg: string;
	id: string;
	...
	FillData() {
		if (this.callTable == true) {
			this.route.params.subscribe((params: Params) => {
				this.id = params['id'];
				this.subscription = this.service.get("WorkersT", "Update", this.id).subscribe((answer: any) => {
					this.model = answer;
	...
	onSubmit() {
		...
		this.data.WorkersID = this.updateForm.get("WorkersID").value;
		...
		this.service.post("WorkersT", "Update", this.data)
