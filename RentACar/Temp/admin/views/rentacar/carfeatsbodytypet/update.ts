...
export class AdminCarFeatsBodyTypeTUpdateComponent {
	errorMsg: string;
	id: string;
	...
	FillData() {
		if (this.callTable == true) {
			this.route.params.subscribe((params: Params) => {
				this.id = params['id'];
				this.subscription = this.service.get("CarFeatsBodyTypeT", "Update", this.id).subscribe((answer: any) => {
					this.model = answer;
	...
	onSubmit() {
		...
		this.service.post("CarFeatsBodyTypeT", "Update", this.data)
